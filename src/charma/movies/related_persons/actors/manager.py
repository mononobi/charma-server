...
from charma.movies.models import Movie2ActorEntity
from charma.movies.related_persons.actors import RelatedActorsPackage
from charma.movies.related_persons.actors.exceptions import Movie2ActorDoesNotExistError
...
class RelatedActorsManager(Manager, NormalizerMixin):
    ...
    def delete(self, movie_id, person_id, **options):
        ...
        store = get_current_store()
        return store.query(Movie2ActorEntity)\
    ...
    def exists(self, movie_id, **options):
        ...
        store = get_current_store()
        return store.query(Movie2ActorEntity.movie_id)\
            .filter(Movie2ActorEntity.movie_id == movie_id).existed()
